...
const ReceiverMessage = ({ message }) => {
  ...
  return (
   ...
   <div ref={scroll} className= 'w-fit max-w-[60%] bg-[#1a1f1f] rounded-t-2xl rounded-bl-2xl rounded-bl-0 px-[10px] py-[10px] relative  left-0 flex flex-col gap-[10px]'>
       {message.image &&
       ...
       {message.message && <div className='text-[18px] text-white wrap-break-word'>
        ...
        <div className='w-[30px] h-[30px] rounded-full cursor-pointer overflow-hidden absolute left-[-25px] bottom-[-40px]'>
          <img src={selectedUser.profileImage} alt="" className='w-full object-cover h-full'/>
        </div>
    </div>
   </div>
  )
