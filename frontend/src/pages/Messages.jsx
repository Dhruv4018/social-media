  ...
  return (
    <div className='w-full min-h-[100vh] flex flex-col bg-black gap-[20px] p-[10px]'>
      <div
        ...
        <MdOutlineKeyboardBackspace className="text-2xl text-white cursor-pointer lg:hidden" onClick={() => navigate("/")} />
        ...
        {prevChatUsers?.map((user, index) => (
          <div className='text-white cursor-pointer w-full flex items-center gap-[10px]' key={index} onClick={() => {
            ...
            {onlineUsers?.includes(user._id) ? (
              ...
              <div className='w-[50px] h-[50px] border-2 border-gray-700 rounded-full cursor-pointer overflow-hidden'>
                <img src={user?.profileImage || dp} alt={user?.name || "user"} className="w-full h-full object-cover" />
              </div>
            )}
